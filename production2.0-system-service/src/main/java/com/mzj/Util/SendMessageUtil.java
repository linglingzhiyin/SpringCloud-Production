...
public class SendMessageUtil {
	...
	public static Integer send(String Uid, String Key, String sendPhoneNum, String desc) {
		...
		// 设置参数
		NameValuePair[] data = { new NameValuePair("Uid", Uid), new NameValuePair("Key", Key), // 秘钥
				new NameValuePair("smsMob", sendPhoneNum), new NameValuePair("smsText", desc) };
	...
	/**
	 * -1 没有该用户账户 -2 接口密钥不正确 [查看密钥]不是账户登陆密码 -21 MD5接口密钥加密不正确 -3 短信数量不足 -11 该用户被禁用
	 * -14 短信内容出现非法字符 -4 手机号格式不正确 -41 手机号码为空 -42 短信内容为空 -51 短信签名格式不正确接口签名格式为：【签名内容】
	 * -6 IP限制 大于0 短信发送数量 以上作为补充
	 */
